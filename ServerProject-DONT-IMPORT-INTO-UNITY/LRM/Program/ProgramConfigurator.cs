    ...
    public partial class Program
    {
        private void ConfigureTransport(Assembly asm)
        {
            ...
            transport.OnServerConnected = (clientID) =>
            {
                ...
                Program.ClientData clientData = new ClientData();
                clientData.clientID = clientID;

                _currentConnections.Add(clientData);
                ...
                if (conf.MaximumConnectionsPerIPAddress > 0)
                {
                    int curMatchCount = 0;
                    string curAddress = transport.ServerGetClientAddress(clientID).Split("]"[0])[0];
                    List<Program.ClientData> playerList = _currentConnections;
                    for (int i = 0; i < playerList.Count; i++)
                    {
                        if (transport.ServerGetClientAddress(playerList[i].clientID).Contains(curAddress))
                        {
                            curMatchCount += 1;
                        }
                    }
                    if (curMatchCount > conf.MaximumConnectionsPerIPAddress)
                    {
                        WriteLogMessage("Client: " + clientID.ToString() + " has been kicked, too many connections from the same ip address.");
            ...
            transport.OnServerDisconnected = (clientID) =>
            {
                ...
                {
                    if (_currentConnections[i].clientID == clientID)
                    {
                        _currentConnections.RemoveAt(i);
                        i = _currentConnections.Count;
                    }
                }
