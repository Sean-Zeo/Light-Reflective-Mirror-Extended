...
namespace LightReflectiveMirror.Endpoints
{
    ...
    public class Endpoint
    {
        ...
        public async Task ServerListCompressed(IHttpContext context)
        {
            ...
            if (Program.conf.EndpointServerList)
            {
                string lobbyName = context.Request.QueryString["lobbyName"];
                List<Room> correctRooms = new List<Room>();
                for (int i = 0; i < _rooms.Count; i++)
                {
                    if (_rooms[i].lobbyName == lobbyName)
                        correctRooms.Add(_rooms[i]);
                }

                string _cachedServerList2 = JsonConvert.SerializeObject(correctRooms, Formatting.Indented);
                string _cachedCompressedServerList2 = _cachedServerList2.Compress();

                await context.Response.SendResponseAsync(_cachedCompressedServerList2);
            }
            else
                await context.Response.SendResponseAsync(HttpStatusCode.Forbidden);
        }
