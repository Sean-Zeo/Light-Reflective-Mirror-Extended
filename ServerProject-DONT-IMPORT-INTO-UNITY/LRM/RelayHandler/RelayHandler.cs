...
namespace LightReflectiveMirror
{
    public partial class RelayHandler
    {
        ...
        private void SendPlayerAccountInfo(int clientId, int playerClientID, string playerName, int playerGroupID, bool playerIsModerator)
        {
            int pos = 0;
            byte[] sendBuffer = _sendBuffers.Rent(900);

            sendBuffer.WriteByte(ref pos, (byte)OpCodes.GetPlayerInfo);
            sendBuffer.WriteInt(ref pos, playerClientID);
            sendBuffer.WriteString(ref pos, playerName);
            sendBuffer.WriteInt(ref pos, playerGroupID);
            sendBuffer.WriteBool(ref pos, playerIsModerator);
